...
% \makeheaddetailssymbol (used inside \extrainfo below) is only defined by the
% casual/banking header variants; provide a fallback bullet separator for styles
% (classic, oldstyle, fancy, contemporary) whose header files don't define it.
\providecommand{\makeheaddetailssymbol}{\hspace{1em}{\rmfamily\textbullet}\hspace{1em}}
...
% moderncv's banking style header (moderncvheadiii) does not implement \photo at all,
% so the photo is placed manually as a page-1-only absolute overlay in the top-right
% margin (outside the centered 0.8\textwidth header block) rather than via \photo.
\usepackage{tikz}
...
\social[googlescholar][scholar.google.com/citations?user=K-zTRIEAAAAJ]{Google Scholar}
...
\cventry{2022--Present}{PhD Research Scholar}{Department of Bioinformatics, University of North Bengal}{Siliguri, India}{}{%
	...
	\begin{itemize}
		...
		\item[\textbullet] Sequence based generative modeling of protein binder generation using autoregressive approaches.
